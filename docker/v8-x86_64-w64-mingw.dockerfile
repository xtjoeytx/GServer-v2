...
RUN \
	...
	&& cd /tmp \
        ...
        && wget https://mirror.msys2.org/mingw/mingw64/mingw-w64-x86_64-openssl-3.1.4-1-any.pkg.tar.zst -O /tmp/mingw-w64-openssl-devel.tar.zst \
	&& tar xvf /tmp/mingw-w64-v8.tar.zst \
        ...
        && tar xvf /tmp/mingw-w64-openssl-devel.tar.zst \
        && mv -fv mingw64 v8 \
	&& rm -rf *.zst \
