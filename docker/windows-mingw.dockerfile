...
RUN git clone https://github.com/microsoft/vcpkg $VCPKG_ROOT \
	...
	&& cd /tmp/gserver \
	&& cmake -GNinja -S/tmp/gserver -B/tmp/gserver/build --preset "Release x64" -DVCPKG_TARGET_TRIPLET:STRING=x86-mingw-static -DSTATIC=ON -DVER_EXTRA=${VER_EXTRA} -DWOLFSSL=ON -DCMAKE_CXX_FLAGS_RELEASE="-O3 -ffast-math" \
