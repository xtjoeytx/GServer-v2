#ifndef VIEWS_CONCAT_LOADER
#define VIEWS_CONCAT_LOADER

#include <version>

// If our STL supports the C++26 std::views::concat, just use that one.
...
namespace xo
{  // exposition only things (and persevering face)
...
public:
	...
	(viewable_range<V> && ...)  //
		...
		operator()(V&&... v)
		const
	{  // noexcept(noexcept(concat_view{static_cast<V&&>(v)...})) {
