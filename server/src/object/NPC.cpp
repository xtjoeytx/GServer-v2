...
void NPC::saveNPC()
{
	...
	*/
	...
	fileData << "HP " << CString(m_character.hitpointsInHalves / 2.0f) << NL;
	...
	for (int i = 0; i < 30; i++)
	{
		if (!m_character.ganiAttributes[i].empty())
...
bool NPC::loadNPC(const CString& fileName)
{
	...
	while (fileData.bytesLeft())
	{
		...
		{
			m_character.hitpointsInHalves = 2 * (int)strtofloat(curLine.readString(""));
		...
		{
			m_character.bowImage = curLine.readString("").toString();
			m_modTime[NPCPROP_GANI] = updateTime;
		}
		else if (curCommand == "HEAD")
		{
			m_character.headImage = curLine.readString("").toString();
			m_modTime[NPCPROP_HEADIMAGE] = updateTime;
		}
		else if (curCommand == "BODY")
		{
			m_character.bodyImage = curLine.readString("").toString();
			m_modTime[NPCPROP_BODYIMAGE] = updateTime;
		}
		else if (curCommand == "SWORD")
		{
			m_character.swordImage = curLine.readString("").toString();
			m_modTime[NPCPROP_SWORDIMAGE] = updateTime;
		}
		else if (curCommand == "SHIELD")
		{
			m_character.shieldImage = curLine.readString("").toString();
			m_modTime[NPCPROP_SHIELDIMAGE] = updateTime;
		}
		else if (curCommand == "HORSE")
		{
			m_character.horseImage = curLine.readString("").toString();
		...
		else if (curCommand.subString(0, 4) == "ATTR")
		{
			...
			{
				int idx = attrId - 1;
				m_character.ganiAttributes[idx] = curLine.readString("").toString();
