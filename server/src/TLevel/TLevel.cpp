...
TLevel::TLevel(short fillTile, IMain* pServer)
:
server(pServer), modTime(0), levelSpar(false), levelSingleplayer(false), levelMap(nullptr), mapx(0), mapy(0)
#ifdef V8NPCSERVER
, _scriptObject(nullptr)
...
/*
	TLevel: Find Level
*/
TLevel* TLevel::findLevel(const CString& pLevelName, IMain* server, bool loadAbsolute)
{
...
/*
	TLevel: Find Level
*/
TLevel* TLevel::createLevel(short fillTile, IMain* server)
{
	std::vector<TLevel*>* levelList = server->getLevelList();

	// Load New Level
	auto *level = new TLevel(fillTile, server);
...
bool TLevel::saveLevel(const std::string& filename) {
	...
	for (const auto& npc : *getLevelNPCs()) {
		if ( npc->getType() != NPCType::LEVELNPC)
