...
void Server::handlePM(Player* player, const CString& message)
{
	if (!m_pmHandlerNpc)
	{
		...
		player->sendPacket({PLO_PRIVATEMESSAGE, CString() >> (short)m_npcServer->getId() << "\"\"," << npcServerMsg.gtokenize()});
		return;
	}
...
void Server::sendPacketToLevelArea(const PlayerOutPacket& packet, std::weak_ptr<Level> level, const std::set<uint16_t>& exclude, PlayerPredicate sendIf) const
{
...
void Server::sendPacketToLevelArea(const PlayerOutPacket& packet, std::weak_ptr<Player> player, const std::set<uint16_t>& exclude, PlayerPredicate sendIf) const
{
...
void Server::sendPacketToOneLevel(const PlayerOutPacket& packet, std::weak_ptr<Level> level, const std::set<uint16_t>& exclude) const
{
	auto levelp = level.lock();
	if (!levelp) return;
